...
/**
 * Drop duplicate product ids, keeping the record with the lowest price.
 * Order of first appearance is preserved; on equal prices the first record wins.
...
export function dedupeBest(records: DealRecord[]): DealRecord[] {
  ...
  records.forEach((rec, index) => {
    const key = rec.productId || `__no_id_${index}`;
    const existing = best.get(key);
    if (existing) {
      if (rec.priceEur < existing.priceEur) best.set(key, rec);
    } else {
      best.set(key, rec);
      order.push(key);
    }
